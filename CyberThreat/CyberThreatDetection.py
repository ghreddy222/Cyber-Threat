...
# from PIL import Image not able to do


#designing main screen
main = tkinter.Tk()
main.title("The Vulnerability of Cyber System Under Stealthy Attacks") 
...
def upload():
 ...
 #tfidf algorithm -->term frequency-inverse document frequency  
def tfidf():
    global X
    global feature_extraction
...
#Combination of LSTN and CNN -->Deeplearning algorithms
def neuralNetwork():
    #text.delete('1.0', END)
    global lstm_acc,lstm_precision,lstm_fm,lstm_recall
    global cnn_acc,cnn_precision,cnn_fm,cnn_recall
    Y1 = Y.reshape((len(Y),1)) #67 used shape --> used for only LSTM algorithm
...
def randomForest():
    ...
    #     if y_test[i]==1 and prediction_data[i]==1:
    #         TP=TP+1
        
    #     elif y_test[i]==1 and prediction_data[i]==0:
    #         FP=FP+1
    #     elif y_test[i]==0 and prediction_data[i]==0:
    #         TN=TN+1
    #     elif y_test[i]==0 and prediction_data[i]==1:
    #         FN=FN+1

    random_con=confusion_matrix(cls,y_test,prediction_data)

    random_cls=classification_report(y_test,prediction_data)


    # text.insert(END,"Random Forest tn : "+str(TP)+"\n")
    ...
    text.insert(END,"Random Forest classification : "+"\n"+str(random_cls)+"\n")
...
def naiveBayes():
    ...
    nb_con=confusion_matrix(y_test,prediction_data)
    ...
    text.insert(END,"Naive Bayes TP : "+str(TP)+"\n")
    text.insert(END,"Naive Bayes TN : "+str(TN)+"\n")
    text.insert(END,"Naive Bayes FP : "+str(FP)+"\n")
    text.insert(END,"Naive Bayes FN : "+str(FN)+"\n")
...
def decisionTree():
    ...
    dt_con=confusion_matrix(y_test,prediction_data)
    ...
    text.insert(END,"Decision Tree confusion matrix : "+"\n"+str(dt_con)+"\n")
...
font = ('times', 16, 'bold')
# img = PhotoImage(file="gitaml.jpg")
# myImage = Image.open("gitaml.jpg")
# myImage.show()


title = Label(main, text='The Vulnerability of Cyber System Under Stealthy Attacks')
title.config(bg='brown4', fg='white')  
